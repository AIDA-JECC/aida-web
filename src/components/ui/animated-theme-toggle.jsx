...
export const AnimatedThemeToggle = ({ className }) => {
  ...
  return (
    <Button
      ...
      className={cn("h-[2.75rem] w-[2.75rem] p-0 rounded-full backdrop-blur-xl border border-neutral-800 hover:border-red-600/70 shadow-xl transition-all duration-300 cursor-pointer bg-neutral-950/90 text-white flex items-center justify-center shrink-0", className)}
      variant="outline"
      aria-label="Toggle Theme Mode"
      title={isDark ? "Switch to Light Mode" : "Switch to Dark Mode"}
    >
      {isLoading ? <CircularProgressSpinner /> : <SolarSwitch isDark={isDark} />}
    </Button>
  );
};
...
const CircularProgressSpinner = () => {
  return (
    <motion.svg
      ...
      className="text-red-500 animate-spin shrink-0"
    >
      <circle
        cx="12"
        cy="12"
